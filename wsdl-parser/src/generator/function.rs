...
use crate::parser::port_type::OperationType::{RequestResponse, OneWay};
...
impl<'a> Param<'a> {
    pub fn new(part: &'a Part<'_>) -> Self {
        Param {
            name: part.name(),
            typename: part.element().or(part.type_()).expect("Element or type must be presented in wsdl:part")
        }
    }
}
...
fn get_input_params<'a>(input: Option<&'a Message<'_>>) -> Vec<Param<'a>> {
    ...
    if input.parts().len() == 1 {  // ONVIF case
        result.push(
            Param {
                name: "request",
