...
pub fn parse_document<'a>(doc: &'a Document)  -> Result<Schema<'a>, String> {
    ...
    for attr in schema_node.attributes() {
        match attr.name() {
            ...
            "attributeFormDefault" => {schema.attribute_form_default = FormChoice::parse(attr.value())?}
...
mod test {
    ...
    fn test_parse_document() {
        ...
        assert_eq!(schema.target_namespace.unwrap().0, "http://www.onvif.org/ver10/schema");
