use crate::xsd_model::complex_types::named_group::{NamedGroup, ContentChoice};
use roxmltree::Node;
...
impl<'a> NamedGroup<'a> {
    pub fn parse(node: Node<'a, '_>) -> Result<Self, String> {
        ...
        let content_choice = content_choice.ok_or_else(|| format!("Content required for xsd:namedGroup type: {:?}", node))?;
        ...
        let name = name.ok_or_else(|| format!("Name required for xsd:namedGroup type: {:?}", node))?;
...
impl<'a> ContentChoice<'a> {
    pub fn parse(node: Node<'a, '_>, xsd_type: ElementType) -> Result<Self, String> {
        match xsd_type {
            ...
            _ => Err(format!("Invalid content type of xsd:namedGroup: {:?}", node))
        }
    }
}
