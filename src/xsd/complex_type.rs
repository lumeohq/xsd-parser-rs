pub mod xsd {
    use std::fmt;
    use crate::xsd::utils::xsd::*;
    use crate::xsd::elements::xsd::*;
    ...
    impl Attribute {
        pub fn new(node: & roxmltree::Node) -> Self {
            let get_type = ||  match_type(node.attribute("type").expect("Type required for attribute")).to_string();
            ...
            Attribute {
                name: lowercase_first_letter(node.attribute("name").unwrap()),
                type_,
    ...
    impl Element {
        pub fn new(node: & roxmltree::Node) -> Self {
            ...
            let t = node.attribute("type").expect("Type required for element");
